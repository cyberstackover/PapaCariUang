...
import com.mygdx.buildthetower.Screens.LevelScreen1;
...
public class Level12 implements Screen {
    ...
    private static final String LEVEL = "12";
    ...
    public Level12(BuildTheTower app) {
        this.app = app;
    }
    ...
    private void setupCount() {
        ...
        {
            app.batch.begin();
            app.font2.draw(app.batch, "x " + numCone, 78, 560);
            app.batch.end();
        }
        ...
        rotate.addListener(new InputListener()
        {
            ...
            public void touchUp(InputEvent event, float x, float y, int pointer, int button)
            {
                if(state == State.play) {
                    ...
                    cone.addAction(Actions.rotateBy(-90,.3f, Interpolation.pow3));
    ...
    private void checkComplete() {
        ...
        {
            checkTime = System.currentTimeMillis();
            if(numCrate == 0 && numCinder == 0 && numBeam == 0 && numCone == 0)
            {
                checking = true;
            }
    ...
    private void setupStars() {
        stars[0]=addStar(world,145,230,0);
        stars[2]=addStar(world,202,240,2);
        stars[3]=addStar(world,255,240,3);
        stars[4]=addStar(world,311,240,4);
        stars[1]=addStar(world,363,230,1);
    }
    ...
    private void updateStage() {
        ...
        if(coneTouched)
        {
            ...
            cone.setPosition(cone.getX()+unprojected.x-startLocationX, cone.getY() + unprojected.y - startLocationY);
    ...
    private void stageListener()
    {
        stage.addListener(new InputListener()
        {
            ...
            public boolean touchDown(InputEvent event, float x, float y,
                                     int pointer, int button) {
                if(x > 0 && x < 87 && y > 573 && y < 655 && numBeam > 0 && !beam.hasActions()) {
                    float scale = .8f;
                    beamTouched = true;
                ...
                if(x > 0 && x < 87 && y > 500 && y < 572 && numCone > 0 && !cone.hasActions()) {
                    float scale = .5f;
                    coneTouched=true;
                    cone.addAction(Actions.scaleTo(2*scale,2*scale));
                    cone.addAction(Actions.alpha(.4f));
                    startLocationX = stage.getViewport().unproject(new Vector2(Gdx.input.getX(),0f)).x;
                    startLocationY = stage.getViewport().unproject(new Vector2(0f,Gdx.input.getY())).y;

                }

                if(x > 0 && x < 87 && y > 658 && y < 723 && numCinder > 0 && !cinder.hasActions()) {
                    float scale = .5f;
                    cinderTouched=true;
    ...
    private void makeCone() {
        ...
        cone.addAction(Actions.alpha(1f));

        if(!(dropLocation.x < 80 && dropLocation.y > 495) && dropLocation.y > 100) {
            ...
            bodies[count] = addCone(world,cone.getX()+cone.getWidth()/2,cone.getY()+cone.getHeight()/2,cone.getWidth()*scale,cone.getHeight()*scale,false);
            ...
            //Checks if bodies are placed outside of screen.
            if(bodies[count].getPosition().x*PPM - cone.getWidth()*scale < 0)
            {
                bodies[count].setTransform(0,bodies[count].getPosition().y,0);
            }
            if(bodies[count].getPosition().x*PPM > 480 - cone.getWidth()*scale)
            {
                bodies[count].setTransform(480/PPM,bodies[count].getPosition().y,0);
            }

            bodies[count].setTransform(bodies[count].getPosition().x, bodies[count].getPosition().y, (float) (cone.getRotation() * (Math.PI / 180)));

            realBox[count] = new Image(Assets.cone);
            realBox[count].setScale(scale);
            realBox[count].setOrigin(Assets.cone.getWidth() / 2, Assets.cone.getHeight() / 2);
            realBox[count].setPosition(bodies[count].getPosition().x * PPM - cone.getWidth(), bodies[count].getPosition().y * PPM - cone.getHeight());
        ...
        cone.setPosition(30,520);
        cone.setOrigin(Assets.cone.getWidth()/4,Assets.cone.getHeight()/4);
        cone.setSize(Assets.cone.getWidth()/2,Assets.cone.getHeight()/2);
        cone.setScale(1);
        cone.setTouchable(Touchable.disabled);
        coneTouched = false;
    }
    ...
    {
        cone = new Image(Assets.cone);



        cone.setPosition(15,1000);
        cone.setOrigin(Assets.cone.getWidth()/4,Assets.cone.getHeight()/4);
        cone.setSize(Assets.cone.getWidth()/2,Assets.cone.getHeight()/2);

        stage.addActor(cone);

        cone.addAction(Actions.scaleTo(0,0,0));


        cone.addAction(Actions.sequence(Actions.delay(1.5f),Actions.parallel(Actions.scaleTo(1f,1f,.5f), Actions.moveTo(30,520,.5f))));
    }
    ...
    private Body addCone(World world, float x, float y, float width, float height, boolean isStatic) {
        Body body;
        BodyDef bdef = new BodyDef();
        bdef.position.set(x/PPM,y/PPM);
        ...
        v2[0] = new Vector2(-67/PPM/4,-41/PPM/2);
        v2[1] = new Vector2(67/PPM/4,-41/PPM/2);
        v2[2] = new Vector2(67/PPM/4,-34/PPM/2);
        v2[3] = new Vector2(-67/PPM/4,-34/PPM/2);
        ...
        v2[0] = new Vector2(-67/PPM/4 + 13/PPM/2,-41/PPM/2 + 7/PPM/2);
        v2[1] = new Vector2(-67/PPM/4 + 29/PPM/2,-41/PPM/2 + 83/PPM/2);
        v2[2] = new Vector2(-67/PPM/4 + 37/PPM/2,-41/PPM/2 + 83/PPM/2);
        v2[3] = new Vector2(-67/PPM/4 + 53/PPM/2 ,-41/PPM/2 + 7/PPM/2);
        shape.set(v2);
        fdef.density = .7f;
        fdef.shape = shape;
