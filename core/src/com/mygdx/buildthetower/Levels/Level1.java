...
public class Level1 implements Screen {
    ...
    public Level1(BuildTheTower app) {
        this.app = app;
    }
                    ...
                    if (forwardBounds.contains(x, y)) {
                        if (starsTouched[0] > 0 || starsTouched[1] > 0 || starsTouched[2] > 0 || starsTouched[3] > 0 || starsTouched[4] > 0) {
                            ...
                            if (!forward.hasActions()) {
                                forward.addAction(sequence(Actions.scaleTo(1.2f, 1.2f, .8f),
    ...
    private void setupStars() {
        stars[0]=addStar(world,app.VIRTUAL_WIDTH/2,app.VIRTUAL_HEIGHT/2,0);
        stars[1]=addStar(world,200,200,1);
        stars[2]=addStar(world,200,350,2);
        stars[3]=addStar(world,150,350,3);
        stars[4]=addStar(world,200,300,4);
    }
    ...
    private void updateStage() {
        ...
        for(int i = 0; i < realBox.length; i++)
        {
            if(realBox[i] != null)
            {
                realBox[i].setPosition(bodies[i].getPosition().x*PPM-crate.getWidth(),bodies[i].getPosition().y*PPM-crate.getHeight());
    ...
    private void setupCrate() {
        ...
        crate.addListener(new InputListener()
        {
            public void touchUp(InputEvent event, float x, float y,
                ...
                if(state == State.play && !crate.hasActions())
                {
                    ...
                    if (!(dropLocation.x < 80 && dropLocation.y > 720)  && dropLocation.y > 100) {
            ...
            public boolean touchDown(InputEvent event, float x, float y,
                ...
                if(state ==  State.play && !crate.hasActions()) {
                    ...
                    crate.addAction(Actions.alpha(.3f));
                    startLocationX = stage.getViewport().unproject(new Vector2(Gdx.input.getX(), 0f)).x;
                    startLocationY = stage.getViewport().unproject(new Vector2(0f, Gdx.input.getY())).y;
                    return true;
                }
        ...
        crate.addAction(sequence(Actions.parallel(Actions.scaleTo(1,1,.5f), Actions.moveTo(15,735,.5f))));
    }
